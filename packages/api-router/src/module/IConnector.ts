...
export type TData = string | Object | Array<any> | Buffer | Stream;
...
export interface IConnector<T extends IConnection = IConnection> {
    // type: string;
    registerApi(basePath: string, apiPath: string, method: string, processer: { (connection: T): Promise<IResult> }): void;
}